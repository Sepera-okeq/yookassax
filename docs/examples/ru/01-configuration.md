...
* [Клиент и цикл событий](#клиент-и-цикл-событий)
...
## Клиент и цикл событий

Асинхронный клиент держит пул соединений, а пул привязан к тому циклу событий,
в котором открылся: сокеты живут в его транспортах. Переиспользовать клиент в
другом цикле нельзя — первый же запрос упадёт с `RuntimeError: Event loop is
closed`.

В обычном приложении это незаметно: цикл один на всё время жизни процесса. Но
Celery запускает каждую задачу через `asyncio.run`, то есть в своём цикле,
который тут же закрывает.

```python
# Так нельзя: клиент переживёт цикл первой задачи.
_client = AsyncYooKassa(oauth_token=token)


@celery_app.task
def charge():
    asyncio.run(_charge())          # второй вызов упадёт
```

Клиент должен жить не дольше цикла:
...
Если клиенты кешируются (например, по одному на магазин), кеш нужно раскладывать
по циклам и закрывать до того, как цикл закроется:
...
Синхронного клиента это не касается: у него нет цикла событий.
